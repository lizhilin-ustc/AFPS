...
def inference(net, config, test_loader, model_file=None):
    ...
    with torch.no_grad():
        ...
        for _data, _label, temp_anno, vid_name, vid_num_seg in test_loader:
            ...
            # FORWARD PASS
            cas, cas_fg, _, discrim, (action_flow, action_rgb) = net(_data, config)
            ...
            # class prediction
            cas_top = torch.gather(cas_fg, 1, topk_indices)
            ...
            if len(pred) != 0:
                ...
                cas_pred = np.reshape(cas_pred, (config.num_segments, -1, 1))      # [750, 1, 1]
                cas_pred = misc_utils.upgrade_resolution(cas_pred, config.scale)   # [18000, 1, 1]
        ...
        json_path = os.path.join(config.model_path, 'temp_result_nms.json')
